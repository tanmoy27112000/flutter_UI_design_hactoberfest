...
class OTPForNumberChange extends StatefulWidget {
  const OTPForNumberChange({Key? key}) : super(key: key);

  @override
  State<OTPForNumberChange> createState() => _OTPForNumberChangeState();
}

class _OTPForNumberChangeState extends State<OTPForNumberChange> {
                      ...
                      onSubmit: (pin) async {
                        try {
                          await FirebaseAuth.instance
                              ...
                              .then((value) async {
                            if (value.user != null) {
                              Get.find<UpdateprofileService>().updateNumber(
                                oldNumber:
                                    sharedPreferences.getString("number"),
                                newNumber: Get.arguments[0],
                              );
                            }
                          });
                        } catch (e) {
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          await FirebaseAuth.instance
            ...
            if (value.user != null) {
              Get.find<UpdateprofileService>().updateNumber(
                oldNumber: sharedPreferences.getString("number"),
                newNumber: Get.arguments[0],
              );
            }
          });
